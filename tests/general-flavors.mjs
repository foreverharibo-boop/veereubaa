...
const fingerprint = `verba-deep-dev-${core.hashText(code)}`;
const saved = { ...defaults, developerMode: true, developerAccessFingerprint: `verba-deep-dev-${core.hashText('091813')}`,
...
for (const id of ['mad-korean', 'hongjin']) {
    assert.match(markup, new RegExp(`id="verba-deep-developer-${id}-enabled" checked`));
    assert.ok(!between('function developerSettingsMarkup(', 'function syncDeveloperQualityControls(').includes(`id="verba-deep-developer-${id}-lab"`));
}
...
assert.ok(index.includes('${generalFlavorSettingsMarkup()}\n\n                <details id="verba-deep-beginner-character-guide"'));
assert.doesNotMatch(markup, /🧪/);
assert.ok(index.indexOf('id="verba-deep-english-flavor"') < index.indexOf('${generalFlavorSettingsMarkup()}'));
...
class Select {}
let saves = 0;
const handler = between("        if (target.id === 'verba-deep-developer-hongjin-enabled'", "        if (target.id === 'verba-deep-developer-relationship-enabled'");
...
for (const [id, key] of [['hongjin', 'developerHongjinFlavorEnabled'], ['mad-korean', 'developerMadKoreanOutputEnabled']]) {
    for (const checked of [false, true]) {
        change(state, Object.assign(new Input(), { id: `verba-deep-developer-${id}-enabled`, checked }), Input, Select, () => saves++, () => {}, {}, { querySelector: () => null });
        assert.equal(state[key], checked);
        assert.equal(state.developerMode, false);
    }
}
assert.equal(saves, 4);
const off = between("        if (target.closest('#verba-deep-developer-mode-off')) {", '            saveSettings();').split('\n').slice(1).join('\n');
